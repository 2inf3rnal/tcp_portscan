...
__   __          _                 _____                   
\ \ / /         | |               /  __ \                  
 \ V /   _ _ __ | | _____ _ __ ___| /  \/_ __ _____      __
  \ / | | | '_ \| |/ / _ \ '__/ __| |   | '__/ _ \ \ /\ / /
  | | |_| | | | |   <  __/ |  \__ \ \__/\ | |  __/\ V  V / 
  \_/\__,_|_| |_|_|\_\___|_|  |___/\____/_|  \___| \_/\_/ 
  {}TCP Port Scanner                 Recoda não comédia!
...
parser = arg.ArgumentParser(description = "TCP Port Scanner by Supr3m0")
parser.add_argument("--ip", action='store', help = " IP / (--ip 192.168.0.1).")
parser.add_argument("--scan", action='store_true', help = "Faz scan da porta 0 até a porta 65.536 (--scan).")
parser.add_argument("--portas", action='store', help = "Faz um scan nas portas que o usuário escolheu (--portas portas.txt).")
parser.add_argument("--threads", action='store', type = int, default = "1",help = "Tempo para cada requisição, também utilizado para burlar firewall (--threads 5).")
parser.add_argument("--verbose", action='store_true', help = "Scanner em tempo real.")
...
if param.portas:
	...
	tipo = "Com base nas portas escolhidas pelo usuário... Portas: {}, {}, {} ... {}".format(portas[0], portas[1], portas[2], portas[-1])
else:
	portas =[]
	faz = [portas.append(porta) for porta in range(0, 65536)]
	tipo = "Todas portas existentes... Portas: {}, {}, {} ... {}".format(portas[0], portas[1], portas[2], portas[-1])

print(index)
print("\n" + F.CYAN + "[+]" + F.WHITE + " Endereço IP:",host)
print(F.CYAN + "[+]" + F.WHITE + " Tipo de scan:",tipo)
print("\n" + F.GREEN + "[*]" + F.WHITE + " Iniciando scan no host {}".format(host))
print()
...
for PORTA in portas:
    ...
    if param.verbose:
    	if retorno == 0:
    		print(F.GREEN + "[ABERTA]" + F.WHITE + " Porta encontra-se aberta:",PORTA)
    		abertas.append(PORTA)
    	else:
    		print(F.RED + "[FECHADA]" + F.WHITE + " Porta encontra-se fechada:",PORTA)
    else:
    	if retorno == 0:
    		print(F.GREEN + "[ABERTA]" + F.WHITE + " Porta encontra-se aberta:",PORTA)
    		abertas.append(PORTA)
...
print(F.GREEN + "\n[+]" + F.WHITE + " Total de portas testadas: ", str(len(portas)))
print(F.GREEN + "[+]" + F.WHITE + " Total de portas abertas: ", str(len(abertas)))
